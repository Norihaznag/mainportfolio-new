...
function makeWhatsAppUrl(appName: string): string {
  const msg = encodeURIComponent(
    `Hello, I am interested in a free trial of ${appName}. Can you contact me?`
  );
  return `https://wa.me/${PHONE}?text=${msg}`;
}

function FaqItem({
  question, answer, open, onToggle, id,
}: {
  ...
  return (
    <div className="border-b border-border-subtle">
      ...
      >
        ...
        <span
          className={`shrink-0 text-ink-faint text-xl leading-none select-none transition-transform duration-200 ${open ? 'rotate-45' : ''}`}
          aria-hidden="true"
        >
          +
        </span>
...
function PricingCard({
  tier, annual, whatsappUrl, isFeatured = false,
}: {
  ...
  return (
    ...
    >
      ...
      <h3 className={`text-xl font-bold mb-1 ${isFeatured ? 'text-white' : 'text-ink'}`}>{tier.name}</h3>

      <div className="flex items-end gap-1.5 mb-1">
        <span className={`text-4xl font-bold tracking-tight ${isFeatured ? 'text-white' : 'text-ink'}`}>
          {price.toLocaleString('en-US')}
        </span>
        <span className={`text-base mb-1 ${isFeatured ? 'text-white/70' : 'text-ink-muted'}`}>MAD/mo</span>
      </div>
      ...
      <a
        ...
        className={`w-full inline-flex items-center justify-center gap-2 font-semibold rounded-lg px-6 py-3 text-[0.9375rem] transition-all duration-200 ${
          isFeatured ? 'bg-white text-accent hover:bg-surface' : 'bg-accent text-white hover:bg-blue-700'
        }`}
      >
        <WhatsAppIcon />
...
export default function AppDetailClient({ app }: { app: DownloadableApp }) {
  ...
  const annualSavingsMonthly = (app.monthlyPrice ?? 0) - (app.annualPrice ?? 0);
  const tiers = app.tiers ?? [];
  const faq = app.faq ?? [];
  const features = app.saasFeatures ?? [];
    ...
    <div className="text-ink">
      ...
      <section className="relative overflow-hidden pt-28 pb-20 px-6" aria-label={`${app.name} — Hero`}>
        ...
        <div className="relative max-w-5xl mx-auto">
          <Link href="/applications" className="inline-flex items-center gap-1.5 text-sm text-ink-muted hover:text-ink transition-colors mb-8 group">
            ...
            <div className="max-w-2xl">
              ...
              {app.monthlyPrice && (
                <div className="inline-flex items-baseline gap-2 bg-surface border border-border-subtle rounded-2xl px-5 py-3 mb-8">
                  ...
                  <span className="text-ink-muted text-sm">MAD/mo</span>
                  {app.annualPrice && (
                    <>
                      <span className="text-xs text-ink-faint mx-2">·</span>
                      <span className="text-sm text-emerald-600 font-medium">{app.annualPrice} MAD/mo billed annually</span>
                    </>
                  )}
                </div>
              )}
        ...
        >
          <div className="max-w-5xl mx-auto">
            ...
            <p className="text-xs text-ink-faint mt-4">
              Contact us for a live personalized demo →{' '}
      ...
      {/* ─── Pricing ──────────────────────────────────────────── */}
      {tiers.length > 0 && app.monthlyPrice && (
        <section className="py-20 px-6 bg-surface border-t border-border-subtle" aria-labelledby="pricing-heading">
          <div className="max-w-5xl mx-auto">
            ...
            {annualSavingsMonthly > 0 && (
              <p className="text-ink-muted mb-8">
                Switch to annual and save{' '}
                <strong className="text-emerald-600">{(annualSavingsMonthly * 12).toLocaleString('en-US')} MAD/year</strong>.
              </p>
            )}
            ...
            >
              ...
              >
                Annual
                <span className="text-xs font-bold text-emerald-600 bg-emerald-100 rounded-full px-2 py-0.5">−20%</span>
              </button>
            </div>
      ...
      {/* ─── Bottom CTA ───────────────────────────────────────── */}
      <section className="py-20 px-6 bg-surface border-t border-border-subtle" aria-label="CTA">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-ink-muted text-[1.0625rem] mb-8">
            Contact our team — we offer a free 14-day trial, no credit card required.
          </p>
          <a
            href={whatsappUrl}
            target="_blank"
